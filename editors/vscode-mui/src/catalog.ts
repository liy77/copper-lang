...
export const WIDGETS: WidgetDef[] = [
  ...
  { name: 'Scroll', doc: 'Scroll viewport.', container: true, props: ['direction', 'gap', 'wheelSpeed', 'dragScroll', 'scrollbar', 'scrollbarColor', 'scrollbarTrackColor', 'scrollbarWidth'] },
  ...
  { name: 'Image', doc: 'Image widget.', positional: true,
    positionalDoc: 'The image source path or http(s) URL (a string); same as `source:`.',
  ...
  { name: 'Dialog', doc: 'Modal-ish overlay: a translucent backdrop plus a centered card holding the children. Use `x:`/`y:` on children to place them inside the card.', container: true,
    props: ['cardWidth', 'cardHeight', 'radius', 'cardColor', 'background', 'bg', 'backdropColor', 'dismissOnBackdrop', 'visible', 'gap'] },
  { name: 'Popup', doc: 'Non-modal floating card: a positioned container (`x:`/`y:`) drawn above its siblings via a high z-index. `visible: false` hides it.', container: true,
    props: ['width', 'height', 'background', 'bg', 'radius', 'borderColor', 'borderWidth', 'shadow', 'padding', 'gap', 'zIndex', 'visible'] },
  ...
  { name: 'Audio', doc: 'Non-visual one-shot sound (a WAV clip). Plays on load unless `autoplay: false`; contributes no widget to the layout.', positional: true,
    positionalDoc: 'The WAV source path (a string); same as `source:`.', props: ['source', 'src', 'volume', 'gain', 'autoplay'] },
...
export const PROP_DOCS: Record<string, string> = {
  ...
  source: 'Media source: a local path, or for Image an `http://`/`https://` URL (downloaded and cached).',
  url: 'The URL to load.',
