...
fn net_module_is_a_crate_no_dep() {
    ...
    assert!(
        o.rust.contains("use copper_net as net;"),
        "alias missing: {}",
        o.rust
    );
    assert!(
        o.rust.contains("use net::"),
        "use net not emitted: {}",
        o.rust
    );
    assert!(
        !o.rust.contains("pub mod net {"),
        "net was inlined: {}",
        o.rust
    );
    assert!(
        o.lib("net").unwrap().contains("pub fn resolve("),
        "resolve not in net lib"
    );
    assert!(
        Parser::std_module_crate_dependencies("net").is_empty(),
        "net has crate deps"
    );
    assert!(
        !o.main_deps.iter().any(|d| d.starts_with("ureq")),
        "net pulled a main dep: {:?}",
        o.main_deps
    );
}
...
fn http_module_is_a_crate_with_ureq_dep() {
    ...
    assert!(
        o.rust.contains("use copper_http as http;"),
        "alias missing: {}",
        o.rust
    );
    ...
    assert!(
        o.lib("http").is_some(),
        "http lib not recorded: {:?}",
        o.lib_crates.iter().map(|(n, _)| n).collect::<Vec<_>>()
    );
    assert!(
        !o.rust.contains("pub mod http {"),
        "http was inlined: {}",
        o.rust
    );
    assert!(
        o.lib("http").unwrap().contains("pub fn get("),
        "get not in http lib"
    );
    // ureq belongs to the http CRATE, not the main project.
    assert!(
        Parser::std_module_crate_dependencies("http")
            .iter()
            .any(|d| d.starts_with("ureq")),
        "ureq dep missing"
    );
    assert!(
        !o.main_deps.iter().any(|d| d.starts_with("ureq")),
...
fn unused_std_modules_not_emitted() {
    let o = transpile("x = 1\nprintln!(\"{}\", x)\n");
    assert!(
        o.lib_crates.is_empty(),
        "modules emitted unprompted: {:?}",
        o.lib_crates
    );
...
fn both_modules_coexist_as_crates() {
    ...
    assert!(
        o.rust.contains("use copper_net as net;") && o.rust.contains("use copper_http as http;"),
        "aliases: {}",
        o.rust
    );
    assert!(
        o.lib("net").is_some() && o.lib("http").is_some(),
        "libs: {:?}",
        o.lib_crates.iter().map(|(n, _)| n).collect::<Vec<_>>()
    );
    assert!(
...
fn url_module_is_a_crate_no_dep() {
    ...
    assert!(
        o.rust.contains("use copper_url as url;"),
        "alias: {}",
        o.rust
    );
    assert!(
        o.lib("url").unwrap().contains("pub fn encode("),
        "encode not in url lib"
    );
    assert!(
        Parser::std_module_crate_dependencies("url").is_empty(),
        "url has crate deps"
    );
}
...
fn json_module_crate_has_serde_dep() {
    ...
    assert!(
        o.rust.contains("use copper_json as json;"),
        "alias: {}",
        o.rust
    );
    assert!(
        o.lib("json").unwrap().contains("pub fn get("),
        "get not in json lib"
    );
    assert!(
        Parser::std_module_crate_dependencies("json")
            .iter()
            .any(|d| d.starts_with("serde_json")),
        "serde_json crate dep missing"
    );
}
...
fn crypto_module_crate_has_sha2_hmac() {
    ...
    assert!(
        o.rust.contains("use copper_crypto as crypto;"),
        "alias: {}",
        o.rust
    );
    assert!(
        o.lib("crypto").unwrap().contains("pub fn sha256("),
        "sha256 not in crypto lib"
    );
    let deps = Parser::std_module_crate_dependencies("crypto");
    assert!(
        deps.iter().any(|d| d.starts_with("sha2")),
        "sha2 crate dep missing"
    );
    assert!(
        deps.iter().any(|d| d.starts_with("hmac")),
        "hmac crate dep missing"
    );
}
...
fn time_fs_ws_are_std_only_crates() {
    for (imp, fn_name, mod_name) in [
        ...
        assert!(
            o.rust
                .contains(&format!("use copper_{mod_name} as {mod_name};")),
            "{mod_name} alias missing: {}",
            o.rust
        );
        assert!(
            o.lib(mod_name)
                .unwrap()
                .contains(&format!("pub fn {fn_name}(")),
            "{fn_name} not in {mod_name} lib"
        );
        assert!(
            Parser::std_module_crate_dependencies(mod_name).is_empty(),
            "{mod_name} has crate deps"
        );
        assert!(
            o.main_deps.is_empty(),
            "{mod_name} pulled a main dep: {:?}",
            o.main_deps
        );
    }
}
